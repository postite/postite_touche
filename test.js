...
TestKeyBoard.prototype = $extend(utest_Test.prototype,{
	KB: null
	,setup: function() {
		this.KB = interact_KeyBoardManager.getInstance();
		var note = new interact_Keynote();
	}
	,testOne: function() {
		utest_Assert.isTrue(true,null,{ fileName : "TestKeyBoard.hx", lineNumber : 17, className : "TestKeyBoard", methodName : "testOne"});
	}
	,testSomething: function(async) {
		$(window.document).on("keydown",null,function(e) {
			utest_Assert.equals(80,e.which,null,{ fileName : "TestKeyBoard.hx", lineNumber : 28, className : "TestKeyBoard", methodName : "testSomething"});
			async.done();
		});
	...
	,testKB: function(async) {
		this.KB.addListener(80,function() {
			utest_Assert.isTrue(true,null,{ fileName : "TestKeyBoard.hx", lineNumber : 37, className : "TestKeyBoard", methodName : "testKB"});
			async.done();
			return;
		});
		this.createEvent(80);
	...
	,testSignal: function(async) {
		...
		this.KB.addListener(77,function() {
			utest_Assert.isTrue(true,null,{ fileName : "TestKeyBoard.hx", lineNumber : 46, className : "TestKeyBoard", methodName : "testSignal"});
			_gthis.KB.signal.handle(function(s) {
				utest_Assert.equals("M",String.fromCharCode(Std.parseInt(s)),null,{ fileName : "TestKeyBoard.hx", lineNumber : 48, className : "TestKeyBoard", methodName : "testSignal"});
				async.done();
				return true;
			});
			return true;
	...
	,createEvent: function(kc) {
		haxe_Log.trace("createEvent" + kc,{ fileName : "TestKeyBoard.hx", lineNumber : 66, className : "TestKeyBoard", methodName : "createEvent"});
...
var interact_KeyBoardManager = function() {
	this.enabled = true;
	this.listeners = new haxe_ds_IntMap();
	this.st = tink_core__$Signal_Signal_$Impl_$.trigger();
	this.signal = this.st;
};
interact_KeyBoardManager.__name__ = ["interact","KeyBoardManager"];
interact_KeyBoardManager.getInstance = function() {
	if(interact_KeyBoardManager._instance == null) {
		interact_KeyBoardManager._instance = new interact_KeyBoardManager();
	}
	return interact_KeyBoardManager._instance;
};
interact_KeyBoardManager.removeInMap = function(it,key,val) {
	if(key != null) {
		it.remove(key);
		return it;
	}
	if(val != null) {
		...
		while(a.hasNext()) {
			var a1 = a.next();
			if(it.get(a1) == val) {
				interact_KeyBoardManager.removeInMap(it,a1);
			}
		}
	}
	return it;
};
interact_KeyBoardManager.prototype = {
	listeners: null
	,enabled: null
	,signal: null
	,st: null
	...
	,addListener: function(key,func) {
		...
		this.listen();
		haxe_Log.trace(this.listeners.toString(),{ fileName : "KeyBoardManager.hx", lineNumber : 52, className : "interact.KeyBoardManager", methodName : "addListener"});
	}
	,removeListener: function(key,func) {
		interact_KeyBoardManager.removeInMap(this.listeners,key,func);
		this.listen();
	}
	,mok: null
	,listen: function() {
		...
		this.restart();
		this.mok = function(e) {
			haxe_Log.trace("key code=" + e.keyCode,{ fileName : "KeyBoardManager.hx", lineNumber : 64, className : "interact.KeyBoardManager", methodName : "listen"});
			var code = e.keyCode;
	...
	,__class__: interact_KeyBoardManager
};
var interact_NoteType = { __ename__ : ["interact","NoteType"], __constructs__ : ["Simple","Sticky"] };
interact_NoteType.Simple = ["Simple",0];
interact_NoteType.Simple.toString = $estr;
interact_NoteType.Simple.__enum__ = interact_NoteType;
interact_NoteType.Sticky = ["Sticky",1];
interact_NoteType.Sticky.toString = $estr;
interact_NoteType.Sticky.__enum__ = interact_NoteType;
var interact_Keynote = function() {
	interact_KeyBoardManager.getInstance().signal.handle($bind(this,this.onTime));
};
interact_Keynote.__name__ = ["interact","Keynote"];
interact_Keynote.createLayout = function() {
	interact_Keynote.lay = window.document.createElement("div");
	interact_Keynote.lay.style.backgroundColor = "#ddd";
	interact_Keynote.lay.style.position = "absolute";
	interact_Keynote.lay.style.zIndex = "999";
	interact_Keynote.lay.style.top = interact_Keynote.lay.style.right = "0";
	interact_Keynote.lay.classList.add("note");
	interact_Keynote.txt = window.document.createElement("p");
	interact_Keynote.lay.appendChild(interact_Keynote.txt);
	window.document.body.appendChild(interact_Keynote.lay);
};
interact_Keynote.removeLast = function() {
	if(interact_Keynote.lay != null) {
		interact_Keynote.lay.remove();
	}
	if(interact_Keynote.timed != null) {
		interact_Keynote.timed.stop();
		interact_Keynote.timed = null;
	}
};
interact_Keynote.prototype = {
	onTime: function(s) {
		haxe_Log.trace("onTime",{ fileName : "Keynote.hx", lineNumber : 26, className : "interact.Keynote", methodName : "onTime"});
		var char = String.fromCharCode(Std.parseInt(s));
		char = char != "" ? char : s;
		this.display("touched" + char,interact_NoteType.Simple);
		return true;
	}
	,note: function(msg,noteType) {
		noteType = noteType != null ? noteType : interact_NoteType.Simple;
	...
	,displaySimple: function(msg) {
		interact_Keynote.removeLast();
		interact_Keynote.timed = new haxe_Timer(1000);
		interact_Keynote.createLayout();
		window.document.body.appendChild(interact_Keynote.lay);
		this.updateMsg(msg);
	}
	,displaySticky: function(msg) {
		if(!interact_Keynote.sticked) {
			interact_Keynote.createLayout();
		}
		this.updateMsg(msg);
	}
	,updateMsg: function(msg) {
		interact_Keynote.txt.innerHTML = msg;
	}
	,__class__: interact_Keynote
};
...
interact_Keynote.sticked = false;
